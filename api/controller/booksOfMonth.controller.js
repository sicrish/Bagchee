        ...
        const data = await prisma.booksOfMonth.findFirst({
            ...
            include: {
                products: {
                    include: {
                        product: {
                            select: { id: true, title: true, price: true, inrPrice: true, realPrice: true, discount: true, defaultImage: true, bagcheeId: true, isbn13: true, isActive: true, synopsis: true, stock: true }
                        }
                    }
                }
            }
    ...
    try {
        const history = await prisma.booksOfMonth.findMany({
            ...
            include: {
                products: {
                    include: {
                        product: { select: { id: true, title: true, price: true, bagcheeId: true, defaultImage: true } }
                    }
                }
            }
        });
