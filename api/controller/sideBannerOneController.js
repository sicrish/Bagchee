...
// SideBannerOne fields: image1, link1, image2, link2, isActive (Boolean), order.
// isActive: 'true' string → Boolean (matches old controller's conversion).
...
export const saveBanner = async (req, res) => {
    try {
        ...
        const newBanner = await prisma.sideBannerOne.create({
...
export const listBanners = async (req, res) => {
    try {
        const data = await prisma.sideBannerOne.findMany({ orderBy: [{ order: 'asc' }, { createdAt: 'desc' }] });
...
export const getBanner = async (req, res) => {
    try {
        const data = await prisma.sideBannerOne.findUnique({ where: { id: parseInt(req.params.id) } });
...
export const updateBanner = async (req, res) => {
    try {
        ...
        const { link1, link2, isActive, order } = req.body;
        const oldBanner = await prisma.sideBannerOne.findUnique({ where: { id } });
...
export const deleteBanner = async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const banner = await prisma.sideBannerOne.findUnique({ where: { id } });
        ...
        await prisma.sideBannerOne.delete({ where: { id } });
