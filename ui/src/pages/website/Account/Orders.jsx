...
const navigate = useNavigate();
  ...
  const getOrderTotal = (order) => {
    ...
    // nets out any cancelled items (out of print / other — mirrors api/lib/orderTotals.js)
    // so the list agrees with the order detail, the invoice and the amount charged.
    // Fallback nets cancelled lines off the stored total.
    if (order.payableTotal != null) return order.payableTotal;
    const cancelled = (order.items || order.products || [])
      .filter((it) => ['cancelled', 'cancelled - other'].includes(String(it.status || '').trim().toLowerCase()))
