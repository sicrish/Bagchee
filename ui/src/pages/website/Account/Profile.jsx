...
const PasswordModal = ({ isOpen, onClose, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(url, { userId, ...passData });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full mt-1 p-3 bg-gray-50 rounded-lg border border-gray-200 outline-none focus:border-primary"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-primary hover:bg-primary-dark text-white py-3 rounded-xl font-bold uppercase tracking-wider mt-4 shadow-lg shadow-primary/20 transition-all"
          >
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (res.data.status) {
        ...
        if(oldAuth) {
            oldAuth.userDetails = { ...oldAuth.userDetails, ...res.data.user };
    ...
    <AccountLayout>
      <div className="max-w-4xl mx-auto">
        ...
        <form
          ...
          className="bg-cream-100 rounded-xl shadow-sm border border-gray-200 overflow-hidden"
        >
          {/* Photo Section */}
          <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8 p-6 md:p-8 bg-cream-100 border-b border-gray-200">
            <div className="relative group shrink-0">
              ...
              <label
                htmlFor="file-upload"
                className="absolute bottom-1 right-1 bg-primary hover:bg-primary-dark text-white p-2 rounded-full shadow-lg cursor-pointer transition-all hover:scale-110 border-2 border-white"
              >
                <Camera size={16} />
              </label>
              <input
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-xs font-bold tracking-wide">
                {selectedFile ? (
                  <span className="text-primary">Photo Selected</span>
                ) : (
          ...
          <div className="px-5 md:px-10 py-6">
            ...
            <div
              ...
              className="group flex flex-col md:flex-row items-start md:items-center justify-between py-5 border-b border-gray-100 px-2 transition-all gap-3 md:gap-0 hover:bg-gray-50 rounded-lg cursor-pointer"
            >
              <div className="w-full md:w-1/3">
                <span className="text-sm font-bold text-gray-600 uppercase tracking-wider">
                  Password
              ...
              <div className="w-full md:w-2/3 flex items-center justify-between gap-4">
                ...
                <div className="text-gray-400 group-hover:text-primary transition-colors p-2 bg-gray-100 rounded-full group-hover:bg-primary-50">
                  <Pencil size={16} />
                </div>
              </div>
            </div>
          ...
          <div className="px-6 py-4 md:py-6 border-t border-gray-200 flex flex-col md:flex-row items-center justify-end gap-3 bg-gray-50">
            <button
              type="submit"
              disabled={loading}
              className="w-full md:w-auto bg-primary hover:bg-primary-hover text-white px-10 py-3 rounded-lg text-sm font-bold uppercase tracking-widest shadow-lg shadow-primary/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
