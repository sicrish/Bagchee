...
const HelpPagesList = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await exportToExcel(dataToExport, "Help Pages", "HelpPages_Report");
            ...
            <tbody className="divide-y divide-cream-50">
              {loading ? (
                ...
                filteredPages.map((page, index) => (
                  <tr key={page.id || page._id} className="hover:bg-primary-50 transition-colors text-[13px]">
                    ...
                    <td className="p-3">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => navigate(`/admin/edit-help-pages/${page.id || page._id}`)} className="p-1.5 bg-cream-50 border border-cream-200 rounded text-text-muted hover:text-primary hover:border-primary transition-all shadow-sm active:scale-95"><Edit size={14} /></button>
                        <button onClick={() => handleDelete(page.id || page._id)} className="p-1.5 bg-cream-50 border border-cream-200 rounded text-text-muted hover:text-red-600 hover:border-red-600 transition-all shadow-sm active:scale-95"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))
