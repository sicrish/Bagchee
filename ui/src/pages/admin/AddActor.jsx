...
const AddActor = () => {
  ...
  const handleSubmit = async (e, actionType) => {
    ...
    setLoading(true);
    const toastId = toast.loading("Saving actor...");
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/actors/save`, data, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });

      if (res.data.status) {
        toast.success("Actor added successfully! 🎭", { id: toastId });
        if (actionType === 'back') {
  ...
  const config = useMemo(() => ({
    ...
    height: 300,
    theme: "default",
    placeholder: 'Enter actor profile details...',
    buttons: [
      ...
      <div className="max-w-6xl mx-auto p-6 mt-4">
        <form className="bg-white rounded border border-cream-200 shadow-sm overflow-hidden">
          ...
          <div className="bg-cream-100 px-6 py-3 border-b border-cream-200">
             <h2 className="text-[11px] font-bold text-text-muted uppercase tracking-wider font-montserrat">Actor Information</h2>
          </div>
