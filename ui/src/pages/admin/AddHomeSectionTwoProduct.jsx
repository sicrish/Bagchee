...
const AddHomeSectionTwoProduct = () => {
  ...
  const [debouncedSearch, setDebouncedSearch] = useState(""); // 🟢 For API Limit protection
  ...
  const handleSubmit = (e, actionType) => {
    ...
    if (!formData.productId) return toast.error("Please select a valid product from dropdown!");
  ...
  return (
    <div className="bg-cream-50 min-h-screen font-body text-text-main pb-20">
              ...
              <div className="col-span-12 md:col-span-9 relative" ref={dropdownRef}>
                  ...
                  <ul className="absolute z-50 w-full bg-white border border-gray-200 mt-1 rounded-xl shadow-2xl max-h-60 overflow-y-auto divide-y divide-gray-50">
                    {isSearching ? (
                        <li className="px-4 py-4 text-sm text-gray-400 italic text-center flex items-center justify-center gap-2">
                             <Loader2 size={14} className="animate-spin" /> Fetching database...
                        </li>
                      ...
                      searchResults.map((p) => (
                        ...
                        >
                          <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-gray-400">Bagchee ID: {p.bagcheeId || p.bagchee_id || p.id}</span>
                          </div>
                ...
                {formData.productId && !showDropdown && (
                  <p className="text-[10px] text-green-600 font-bold mt-2 ml-1 uppercase flex items-center gap-1">
                    <Check size={12}/> Product Loaded: {formData.productId}
                  </p>
                )}
              </div>
            </div>
      ...
      <style>{`.theme-input { width: 100%; border: 1px solid #e5e7eb; border-radius: 6px; padding: 10px; font-size: 0.85rem; transition: border-color 0.2s; } .theme-input:focus { border-color: #41C9E2; outline: none; } .animate-fadeIn { animation: fadeIn 0.4s ease-out; } @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }`}</style>
    </div>
  );
};
