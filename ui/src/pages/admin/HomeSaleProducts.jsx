...
const HomeSaleProducts = () => {
  ...
  // Pagination State (Optional setup for future)
  ...
  // --- 1. FETCH DATA (Paginated for Screen) ---
  const fetchProducts = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}/home-sale-products/list`, { params });
      
      if (res.data.status) {
        setProducts(res.data.data);
        // Agar backend total count bhejta h to yahan set karein
  ...
  }, [pagination.page, pagination.limit]); // Refetch on page change
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const API_URL = process.env.REACT_APP_API_URL;
      const res = await axios.delete(`${API_URL}/home-sale-products/delete/${id}`);
      if (res.data.status) {
  ...
  const clearFilters = () => {
    ...
    // State update hone ke baad fetch effect se call hoga ya manual call karein
    setTimeout(() => fetchProducts(), 100); 
  };

  // --- 🟢 3. EXPORT FULL DATA (Using xlsx & file-saver) ---
  const handleExport = async () => {
    ...
    try {
      ...
      // 🔥 SPECIAL CALL: Fetch ALL data for export (ignoring pagination limit)
      // Backend me hum 'limit=all' ya 'export=true' handle karenge
      const res = await axios.get(`${API_URL}/home-sale-products/list`, { 
  ...
  const handlePrint = () => {
    ...
    // Temporary style for printing to ensure scrolling doesn't hide data
    ...
    document.body.innerHTML = `
      <div style="padding: 20px; font-family: sans-serif;">
        <h1 style="text-align:center; margin-bottom: 20px; color: #333;">Home Sale Products Report</h1>
    ...
    window.location.reload(); // Reload needed to reactivate React event listeners
  };
  ...
  return (
    <div className="bg-cream-50 min-h-screen p-4 md:p-6 font-body text-text-main">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
        <button 
          onClick={() => navigate('/admin/add-home-sale')} 
        ...
        >
          <Plus size={14} className="text-red-600" /> Add Home Sale Products
        </button>
      ...
      <div className="bg-white rounded border border-cream-200 overflow-hidden shadow-sm" id="printable-table-area">
        ...
        <div className="p-4 bg-white border-t border-cream-50 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <button 
                // Logic needed for max pages based on total
                onClick={() => setPagination({...pagination, page: pagination.page + 1})}
