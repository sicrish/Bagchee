...
// 🔄 Change 1: Component Name Updated
const AddHomeSectionThreeProduct = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 🟢 B. Sections layein (Dynamic List)
  ...
  const handleSubmit = async (e, actionType) => {
    ...
    if (!formData.home_section_id) {
        return toast.error("Please select a Target Section from the dropdown!");
    }
    ...
    setLoading(true);
    const toastId = toast.loading("Saving to Section 3..."); // UI Feedback
    ...
    try {
      ...
      if (res.data.status) {
        toast.success("Saved to Section 3! 📦", { id: toastId });
        if (actionType === 'back') {
          // 🔄 Change 2: Redirect to Section 3 List
          navigate('/admin/home-section-3'); 
        } else {
          setSearchTerm("");
          setFormData(prev => ({ 
            ...prev, 
    ...
    <div className="bg-cream-50 min-h-screen font-body text-text-main pb-20">
      ...
      <div className="bg-primary px-6 py-4 shadow-lg flex items-center justify-between text-white sticky top-0 z-50">
        <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold uppercase tracking-wider font-display text-white">Add to Section 3</h1>
        </div>
        {/* 🔄 Change 4: Close Button Link */}
        <button onClick={() => navigate('/admin/home-section-3')}><X size={24}/></button>
      </div>
                ...
                >
                    ...
                    {sectionsList.map((sec) => (
                        <option key={sec._id} value={sec._id}>
                            {sec.title} ({sec.section}) {/* e.g. New & Notable (Section 3) */}
                        </option>
                    ))}
                </select>
                <p className="text-[9px] text-gray-400 mt-2 italic">Select "New & Notable" (Section 3) or any other section.</p>
              </div>
            </div>
...
export default AddHomeSectionThreeProduct;