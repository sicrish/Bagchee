...
const EditTestimonials = () => {
  ...
  // 🟢 Fields matching your image structure
  ...
  // 🟢 1. Load Data directly for Testimonials
  useEffect(() => {
    const fetchTestimonialData = async () => {
      try {
        const API_URL = process.env.REACT_APP_API_URL;
        const res = await axios.get(`${API_URL}/testimonials/get`); 
  ...
  const handleSubmit = async (e, actionType) => {
    ...
    try {
      ...
      if (res.data.status) {
        toast.success("Testimonials updated successfully!", { id: toastId });
  ...
  const config = useMemo(() => ({
    ...
    height: 450,
    theme: "default",
    placeholder: 'Write testimonials content...',
