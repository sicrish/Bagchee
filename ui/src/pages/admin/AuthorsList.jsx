...
const AuthorsList = () => {
  ...
  // 🟢 1. Fetch Authors from Backend
  const fetchAuthors = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/authors/list`);
      if (res.data.status) {
        setAuthors(res.data.data);
      }
    } catch (error) {
      console.error("Fetch Error:", error);
      toast.error(error.response?.data?.msg || "Failed to load authors");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-cream-50 min-h-screen p-4 md:p-6 font-body text-text-main">
      ...
      <div className="mt-6 flex flex-col md:flex-row justify-between items-center gap-4 px-2">
          <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
            Total Record(s): {filteredAuthors.length} / {authors.length}
          </span>
