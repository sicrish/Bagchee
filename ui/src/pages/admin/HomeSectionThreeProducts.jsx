...
// 🔄 Change 1: Component Name -> Section 3
const HomeSectionThreeProducts = () => {
  ...
  const fetchSectionProducts = async () => {
    ...
    try {
      ...
      // 🔄 Change 2: URL -> Section 3 List
      // (Backend route banana padega: /section-three/list)
      const res = await axios.get(`${API_URL}/home-sections/products/section-three/list`); 
      
      // console.log("Section 3 Response:", res.data);
      ...
      toast.error("Failed to load Section 3 products");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    // 🔄 Change 3: Confirmation text
    
    
    const toastId = toast.loading("Removing...");
    try {
      const API_URL = process.env.REACT_APP_API_URL;
      // Delete URL same rahega (Universal)
  ...
  return (
    <div className="bg-cream-50 min-h-screen p-4 md:p-6 font-body text-text-main">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 font-montserrat">
        <button 
          // 🔄 Change 4: Add Button Link -> Section 3 Add Page
          onClick={() => navigate('/admin/add-home-section-3-product')} 
        ...
        >
          {/* 🔄 Change 5: Button Text */}
          <Plus size={14} className="text-red-600" /> Add Home section 3 products
        </button>
              ...
              {loading ? (
                ...
                filteredData.map((item) => (
                  <tr key={item._id} className="hover:bg-primary/5 transition-colors text-[13px]">
                    ...
                    <td className="p-3 border-r border-cream-50 text-text-main font-medium">{typeof item.productId === 'object' ? item.productId?._id : item.productId}</td>
                ...
                <tr>
                  <td colSpan="6" className="p-10 text-center text-text-muted italic">No products found in Section 3.</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default HomeSectionThreeProducts;