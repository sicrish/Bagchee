...
import { Check, RotateCcw, X, Loader2, Calendar } from 'lucide-react';
...
const AddCoupons = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        if (res.data.status) {
          setCategoryList(res.data.data); // Assuming data is array of objects
        }
      } catch (error) {
  ...
  const handleSubmit = async (e, actionType) => {
    ...
    try {
      const API_URL = process.env.REACT_APP_API_URL;
      const res = await axios.post(`${API_URL}/coupons/save`, formData);
