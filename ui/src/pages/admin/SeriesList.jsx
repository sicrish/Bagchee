...
const SeriesList = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await exportToExcel(dataToExport, "Series", "Series_Report");
                  ...
                  <tr key={item.id || item._id} className="hover:bg-primary-50 transition-colors text-[13px]">
                    ...
                    <td className="p-3 text-center">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => navigate(`/admin/edit-series/${item.id || item._id}`)} className="p-1.5 bg-cream-50 border border-cream-200 rounded text-text-muted hover:text-primary hover:border-primary transition-all shadow-sm active:scale-95">
                          <Edit size={14} />
                        </button>
