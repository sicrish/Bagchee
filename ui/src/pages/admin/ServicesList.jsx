...
const ServicesList = () => {
    ...
    // 🟢 1. Filtering States (Based on image_150c3a.png fields)
    ...
    const fetchServices = useCallback(async () => {
        ...
        try {
            ...
            // 🟢 Specific fetch for Services
            const res = await axios.get(`${API_URL}/services/list`);
            if (res.data.status) {
    ...
    const filterInputClass = "w-full rounded p-1 text-xs outline-none bg-white/90 focus:bg-white text-text-main font-montserrat border border-transparent focus:border-primary transition-all placeholder:text-gray-400";
    ...
    return (
        <div className="bg-cream-50 min-h-screen p-4 md:p-6 font-body text-text-main">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                {/* 🟢 Add Service Button with Link Update */}
                <button
                ...
                >
                    <Plus size={14} className="text-red-600" /> Add Service
                </button>
                ...
                <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end">
                    ...
                    <div className="relative flex items-center">
                        <button onClick={fetchServices} className="bg-primary text-white p-2 rounded hover:bg-primary-hover transition-colors shadow-sm">
                            <Search size={16} />
                        </button>
                    </div>
                </div>
            ...
            <div className="bg-white rounded border border-cream-200 overflow-hidden shadow-sm">
                    ...
                    <table className="w-full min-w-[1000px] border-collapse text-sm">
                        ...
                        <tbody className="divide-y divide-cream-50">
                            {loading ? (
                                <tr>
                                    <td colSpan="5" className="p-10 text-center text-text-muted font-bold">
                                        <div className="flex justify-center items-center gap-2 animate-pulse">
                                            <Loader2 className="animate-spin text-primary" /> Loading Services...
                                        </div>
                                    </td>
                                </tr>
                            ) : filteredServices.length > 0 ? (
                                filteredServices.map((item, index) => (
                                    <tr key={item.id || item._id} className="hover:bg-primary-50 transition-colors text-[13px] group">
                                        ...
                                        <td className="p-3">
                                            <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                                                <button
                                                    onClick={() => navigate(`/admin/services/edit/${item.id || item._id}`)}
                                ...
                                <tr>
                                    <td colSpan="5" className="p-20 text-center text-text-muted italic font-montserrat tracking-wide">
                                        No Data found
                                    </td>
                                </tr>
                            )}
                ...
                <div className="p-4 bg-white border-t border-cream-50 flex flex-col md:flex-row justify-between items-center gap-4 font-montserrat">
                    <div className="text-[11px] font-bold text-text-muted uppercase tracking-tighter">
                        Displaying {filteredServices.length} of {services.length} items
                    </div>
