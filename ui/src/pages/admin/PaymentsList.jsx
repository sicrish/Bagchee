...
const PaymentsList = () => {
  ...
  const fetchPayments = async (isExport = false) => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/payments/list?${params.toString()}`);
      if (res.data.status) {
  ...
  const handleExport = async () => {
    ...
    try {
      const allData = await fetchPayments(true);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const res = await axios.delete(`${API_URL}/payments/delete/${id}`);
      if (res.data.status) {
  ...
  return (
    <div className="bg-cream-50 min-h-screen p-4 md:p-6 font-body text-text-main">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
        <button
          onClick={() => navigate('/admin/add-payments')}
        ...
        >
          <Plus size={14} className="text-red-600" /> Add Payments
        </button>
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end">
          ...
          <div className="relative flex items-center">
            <button onClick={fetchPayments} className="bg-primary text-white p-2 rounded hover:bg-primary-hover transition-colors shadow-sm">
              <Search size={16} />
            </button>
          </div>
        </div>
            ...
            <thead>
              ...
              <tr className="bg-primary border-b border-cream-200 align-top">
                ...
                <td className="p-2 text-center">
                  <button onClick={fetchPayments} className="text-white hover:rotate-180 transition-transform duration-500">
                    <RotateCw size={16} />
                  </button>
                </td>
              </tr>
                  ...
                  <tr key={item.id || item._id} className="hover:bg-primary-50 transition-colors text-[13px]">
                    ...
                    <td className="p-3">
                      <div className="flex justify-center gap-2">
                        ...
                        <button onClick={() => handleDelete(item.id || item._id)} className="p-1.5 bg-cream-50 border border-cream-200 rounded text-text-muted hover:text-red-600 hover:border-red-600 transition-all shadow-sm active:scale-95"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))
