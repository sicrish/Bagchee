...
const HomeSectionFourProducts = () => {
  ...
  const fetchSectionProducts = async () => {
    ...
    try {
      ...
      // 🔄 Change 2: URL -> Section 4 List
      // (Backend route banana padega: /section-four/list)
      const res = await axios.get(`${API_URL}/home-sections/products/section-four/list`); 
      
      console.log("Section 4 Response:", res.data);
      ...
      toast.error("Failed to load Section 4 products");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredData = useMemo(() => {
    return products.filter((item) => {
      return (
        (item.productId || "").toString().includes(filters.productId) &&
  ...
  const handleDelete = async (id) => {
    ...
    if(!window.confirm("Are you sure you want to remove this product from Section 4?")) return;
    
    const toastId = toast.loading("Removing...");
    try {
      const API_URL = process.env.REACT_APP_API_URL;
      // Delete URL universal rahega
  ...
  return (
    <div className="bg-cream-50 min-h-screen p-4 md:p-6 font-body text-text-main">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 font-montserrat">
        <button 
          // 🔄 Change 4: Add Button Link -> Section 4 Add Page
          onClick={() => navigate('/admin/add-home-section-4-product')} 
        ...
        >
          ...
          <Plus size={14} className="text-red-600" /> Add Home section 4 products
        </button>
              ...
              {loading ? (
                ...
                filteredData.map((item) => (
                  <tr key={item._id} className="hover:bg-primary/5 transition-colors text-[13px]">
                    ...
                    <td className="p-3 border-r border-cream-50 text-text-main font-medium">{item.productId}</td>
                ...
                <tr>
                  <td colSpan="6" className="p-10 text-center text-text-muted italic">
                    {/* 🔄 Change 7: Empty state text */}
                    No products found in Section 4.
                  </td>
                </tr>
              )}
            </tbody>
...
export default HomeSectionFourProducts;