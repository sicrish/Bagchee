...
const [totalItems, setTotalItems] = useState(0);  
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await exportToExcel(dataToExport, "OrderStatuses", "OrderStatuses");
            ...
            <tbody className="divide-y divide-cream-50">
              {loading ? (
                ...
                filteredStatuses.map((status, index) => (
                  <tr key={status.id || status._id} className="hover:bg-primary-50 transition-colors text-[13px]">
                    ...
                    <td className="p-3">
                      <div className="flex justify-center gap-2">
                        <button 
                          onClick={() => navigate(`/admin/edit-order-status/${status.id || status._id}`)} 
                        ...
                        <button 
                          onClick={() => handleDelete(status.id || status._id)}
