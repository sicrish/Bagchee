...
const SideBySideBannerone = () => {
    ...
    const fetchBanners = async () => {
        ...
        try {
            ...
            // 🟢 API Endpoint changed for Side Banners
            const res = await axios.get(`${API_URL}/side-banner-one/list`, { params });
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const API_URL = process.env.REACT_APP_API_URL;
            // 🟢 Delete Endpoint changed
            const res = await axios.delete(`${API_URL}/side-banner-one/delete/${id}`);
            if (res.data.status) {
    ...
    const handleExport = async () => {
        ...
        try {
            const API_URL = process.env.REACT_APP_API_URL;
            const res = await axios.get(`${API_URL}/side-banner-one/list`, {
            ...
            if (res.data.status && res.data.data.length > 0) {
                ...
                await exportToExcel(exportData, "Side Banners", "side_banners");
    ...
    const handlePrint = () => {
      ...
      <div style="padding: 20px; font-family: sans-serif;">
        <h1 style="text-align:center;">Side By Side Banners Report</h1>
    ...
    return (
        <div className="bg-cream-50 min-h-screen p-4 md:p-6 font-body text-text-main">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                <button
                    onClick={() => navigate('/admin/add-side-banner-one')}
                ...
                >
                    <Plus size={14} className="text-red-600" /> Add Side Banner
                </button>
                    ...
                    <table className="w-full min-w-[800px] border-collapse text-sm">
                        <thead>
                            <tr className="bg-primary text-white border-b border-white/10 font-montserrat font-bold uppercase tracking-wider">
                                ...
                                {/* 🟢 Updated Header for Side by Side Images */}
                            ...
                            {loading ? (
                                ...
                                banners.map((item, index) => (
                                    ...
                                    <tr key={item.id} className="hover:bg-primary-50 transition-colors">
                                        ...
                                        {/* 🟢 BANNER 1 COLUMN (Image + Link) */}
                                        <td className="p-2 border-r border-cream-50 text-text-main">
                                            <div className="flex flex-col gap-1.5">
                                                <div className="w-24 h-12 bg-gray-100 rounded border border-gray-200 overflow-hidden shadow-sm relative group cursor-pointer">
                                                    <img src={getImageUrl(item.image1)} alt="Banner 1" className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://placehold.co/100x50?text=No+Img"; }} />
                                                </div>
                                                <div className="flex items-center gap-1 text-[10px] text-primary font-bold">
                                                    <LinkIcon size={10} />
                                                    <span className="truncate max-w-[150px]" title={item.link1}>{item.link1 || 'No Link'}</span>
                                                </div>
                                            </div>
                                        </td>

                                        {/* 🟢 BANNER 2 COLUMN (Image + Link) */}
                                        <td className="p-2 border-r border-cream-50 text-text-main">
                                            <div className="flex flex-col gap-1.5">
                                                <div className="w-24 h-12 bg-gray-100 rounded border border-gray-200 overflow-hidden shadow-sm relative group cursor-pointer">
                                                    <img src={getImageUrl(item.image2)} alt="Banner 2" className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://placehold.co/100x50?text=No+Img"; }} />
                                                </div>
                                                <div className="flex items-center gap-1 text-[10px] text-accent font-bold">
                                                    <LinkIcon size={10} />
                                                    <span className="truncate max-w-[150px]" title={item.link2}>{item.link2 || 'No Link'}</span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="p-3 text-center hide-on-print">
                                            <div className="flex justify-center gap-2">
                                                <button onClick={() => navigate(`/admin/edit-side-banner-one/${item.id}`)} className="p-1.5 bg-cream-50 border border-cream-200 rounded text-text-muted hover:text-primary hover:border-primary transition-all shadow-sm"><Edit size={14} /></button>
...
export default SideBySideBannerone;