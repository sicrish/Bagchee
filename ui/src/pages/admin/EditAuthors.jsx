...
          if (author.picture) {
            ;
            setPreviewImage(`${API_URL}${author.picture}`);
          }// Purani image dikhane ke liye
        }
      } catch (error) {
        console.error(error);
...
                {/* 🟢 SMART PREVIEW: Nayi file chuni hai to wo dikhao, warna database wali purani photo */}
                {(imageFile || previewImage) && (
                  <div className="relative group">
                    <img
                      src={imageFile ? URL.createObjectURL(imageFile) : previewImage}
                      alt="Author Preview"
                      className="w-16 h-16 object-cover rounded border border-cream-200 shadow-sm"
...
                    <Check size={12} />
                    <span className="text-[10px] italic truncate max-w-[100px]">{imageFile.name}</span>
                  </div>
                )}
              </div>
            </div>
