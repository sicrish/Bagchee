...
const OrdersList = () => {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order, index) => {
      const displayId = (order.order_id || index + 1).toString();
      const orderDate = order.createdAt ? new Date(order.createdAt).toLocaleDateString('en-GB') : "";
      
      return (
        displayId.includes(filters.id) &&
        orderDate.includes(filters.date) &&
        (order.payment_type || "paypal").toLowerCase().includes(filters.type.toLowerCase()) &&
        (order.customer_name || "").toLowerCase().includes(filters.customer.toLowerCase()) &&
        (order.total || 0).toString().includes(filters.total) &&
        (order.status || "not yet ordered").toLowerCase().includes(filters.status.toLowerCase()) &&
      ...
      <div className="bg-white rounded border border-cream-200 overflow-hidden shadow-sm">
              ...
              {loading ? (
                ...
                filteredOrders.map((order, index) => (
                  <tr key={order._id} className="hover:bg-primary-50 transition-colors text-[13px]">
                    ...
                    <td className="p-3 border-r border-cream-50 text-text-main font-bold">{order.customer_name}</td>
                    <td className="p-3 border-r border-cream-50 text-text-main font-bold">${order.total ? order.total.toFixed(2) : '0.00'}</td>
                    ...
                    <td className="p-3 border-r border-cream-50 text-text-main font-medium">{order.payment_status || '-'}</td>
        ...
        <div className="p-4 bg-white border-t border-cream-50 flex flex-col md:flex-row justify-between items-center gap-4 font-montserrat">
          <div className="text-[11px] font-bold text-text-muted uppercase tracking-tighter">
            Displaying {filteredOrders.length} of {orders.length} items
          </div>
