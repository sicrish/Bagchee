...
const AddLanguages = () => {
  ...
  const handleSubmit = async (e, actionType) => {
    ...
    setLoading(true);
    const toastId = toast.loading("Saving language...");

    try {
      const API_URL = process.env.REACT_APP_API_URL;
      const res = await axios.post(`${API_URL}/languages/save`, formData);

      if (res.data.status) {
        toast.success("Language added successfully! 🌐", { id: toastId });
        if (actionType === 'back') {
            ...
            <div className="grid grid-cols-12 gap-4 items-center border-b border-cream-50 pb-4">
              ...
              <div className="col-span-9">
                <input 
                  ...
                  onChange={handleChange} 
                  placeholder="0"
                  className="w-full border-2 border-cream-200 rounded-md px-4 py-2.5 text-[13px] outline-none transition-all focus:border-primary focus:bg-white bg-cream-50/30 text-text-main font-bold" 
                />
              </div>
            </div>
