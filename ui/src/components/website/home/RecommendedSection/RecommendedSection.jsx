...
const RecommendedSection = () => {
  ...
  // 🟢 1. API Call (Fetch Recommended Products)
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Calling the 'getRecommended' controller
        const res = await axios.get(`${API_URL}/product/recommended?page=${page}&limit=${itemsPerPage}`);
        
        if (res.data.status) {
          setProducts(res.data.data);
          // Backend sends 'total' count, use it to calculate total pages
    ...
    <section className="py-10 md:py-16 bg-cream-50 font-body">
      <div className="max-w-[1400px] mx-auto px-4 group/section">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-10 gap-2 md:gap-4 border-b border-primary-200 pb-4">
            ...
            <Link to="/book?isRecommended=true" className="flex items-center gap-2 text-xs md:text-sm uppercase tracking-wider text-text-main hover:text-primary transition-colors self-end md:self-auto font-montserrat">
        ...
        <div className="relative">
            ...
            {/* Left Arrow (Disable if on Page 1) */}
            <button 
                onClick={handlePrev} 
                disabled={page === 1}
            ...
            {/* Right Arrow (Disable if on last page) */}
            <button 
                onClick={handleNext} 
